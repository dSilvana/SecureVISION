...
-- Incoming plaintext stream in 128 bit blocks, this is for handshaking with AES. Not implemented.
    ...
    pt_block_valid : in  std_logic;                    -- 1-cycle pulse when pt_block_in valid when there is a block inside.
    pt_block_in    : in  std_logic_vector(127 downto 0); -- Plaintext block in, connect to AES out
    pt_block_ready : out std_logic;                    -- request next 128-bit block, should tell aes when to send next block
...
-- wrapper outputs digest + pass/fail
    ...
    pass_o      : out std_logic;		       -- outputs pass flag 1
    fail_o      : out std_logic; 		       -- outputs fail flag 1
    ...
    if rising_edge(clk) then
      if rst = '1' then
        ...
        case st is
          ...
          when ST_WAIT_BLOCK =>
            if pt_block_valid = '1' then
              cur_block <= pt_block_in;
